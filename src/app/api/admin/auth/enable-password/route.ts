...
export async function POST(request: Request) {
  try {
    ...
    if (error) {
      console.error("[POST /api/admin/auth/enable-password] DB error:", error);
      return NextResponse.json(
    ...
    await logAdminSecurityEvent({
      ...
      ipAddress: ip,
      userAgent,
      eventType: "admin_password_enabled",
      severity: "info",
    });

    return NextResponse.json({ ok: true, success: true });
  } catch (error) {
    console.error("[POST /api/admin/auth/enable-password] error:", error);
    return NextResponse.json(
