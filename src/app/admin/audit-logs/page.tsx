  ...
  const columns: ColumnDef<AuditLogItem>[] = [
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center space-x-3 py-1 text-[11px] font-black text-foreground/40 tracking-tighter uppercase font-mono whitespace-nowrap">
      ...
      cell: ({ row }) => (
        <div className="flex items-center space-x-4 min-w-0">
          <div className="w-10 h-10 rounded-xl bg-surface-elevated border border-border flex items-center justify-center shadow-sm flex-shrink-0">
            <UserIcon className="w-4 h-4 text-foreground/20" />
          </div>
          <p className="text-[14px] font-black text-foreground tracking-tight leading-none truncate">{row.original.admin}</p>
        </div>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center space-x-4 min-w-0">
          <div className="p-2 rounded-xl bg-primary/10 border border-primary/15 shadow-sm shadow-primary/5 flex-shrink-0">
            <Terminal className="w-4 h-4 text-primary" />
          </div>
          <span className="text-[11px] font-black text-foreground uppercase tracking-widest leading-none truncate">{row.original.action}</span>
        </div>
      ),
    },
    {
      ...
      cell: ({ row }) => <span className="text-[11px] font-black text-foreground/30 uppercase tracking-[0.2em] whitespace-nowrap">{row.original.target}</span>,
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center space-x-3 text-[11px] text-foreground/20 font-black font-mono tracking-tighter uppercase whitespace-nowrap">
    ...
    <DashboardShell>
      <div className="section-spacing">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
           <div className="p-8 rounded-[32px] bg-surface border border-border flex items-center space-x-6 shadow-premium hover:border-primary/20 transition-all cursor-pointer group">
              <div className="w-16 h-16 rounded-[22px] bg-primary/10 flex items-center justify-center border border-primary/20 shadow-lg shadow-primary/10 group-hover:scale-110 transition-transform duration-500 flex-shrink-0">
                 <Shield className="w-7 h-7 text-primary" />
              </div>
              <div>
                 <p className="text-[10px] font-black uppercase tracking-[0.3em] text-foreground/20">Registry Events</p>
                 <h4 className="text-3xl font-black text-foreground tracking-tighter mt-1 leading-none">{totalCount.toLocaleString()}</h4>
              </div>
           </div>

           <div className="p-8 rounded-[32px] bg-surface border border-border flex items-center space-x-6 shadow-premium hover:border-accent-orange/20 transition-all cursor-pointer group">
              ...
              <div>
                 <p className="text-[10px] font-black uppercase tracking-[0.3em] text-foreground/20">Protocol Mutations</p>
                 <h4 className="text-3xl font-black text-foreground tracking-tighter mt-1 leading-none">{mutationsCount}</h4>
              </div>
           </div>

           <div className="p-8 rounded-[32px] bg-surface border border-border flex items-center space-x-6 shadow-premium hover:border-success-green/20 transition-all cursor-pointer group">
              ...
              <div>
                 <p className="text-[10px] font-black uppercase tracking-[0.3em] text-foreground/20">Ecosystem Status</p>
                 <h4 className="text-3xl font-black text-foreground tracking-tighter mt-1 leading-none uppercase tracking-tight">{missingTableInfo.isMissing ? "Missing DDL" : isError ? "Degraded" : "Nominal"}</h4>
              </div>
           </div>
        </div>
